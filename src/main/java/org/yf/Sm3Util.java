...
public class Sm3Util {
     ...
     * @param source 原始数据
     * @return 摘要
     * @throws IOException 填充错误
     */
    public static byte[] digest(byte[] source) throws IOException {
     ...
     * @throws IOException 填充错误
     */
    public static String digestHex(byte[] source) throws IOException{
        return byteArrayToHexString(digest(source));
    }
    ...
    private static int t(int j) {
        if (j >= 0 && j <= 15) {
            ...
            throw new IllegalArgumentException("错误的位数:" + j);
        }
    }
    ...
    private static byte[] padding(byte[] source) throws IOException {
        if (source.length >= 0x2000000000000000L) {
            throw new IllegalArgumentException("错误的位数:" + source.length);
        }
